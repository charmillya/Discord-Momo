...
class cSetBirthday(commands.Cog):
    ...
    async def setbirthday(
        ...
        birthday: str = nextcord.SlashOption(
            required=True,
            name="birthday",
            description="Your birthday! Must respect this format: YYYY-MM-DD (i.e. 2007-07-27)"
        ),
        ):
        ...
        if results[8] != None:
            ...
            await inter.response.send_message(f"{inter.user.mention}, the **format** of the birthday you provided is **incorrect**! Please provide a birthday in the following format: **YYYY-MM-DD** (i.e. 2007-07-27)")
        else:
            if birthday[4] != "-" or birthday[7] != "-":
                await inter.response.send_message(f"{inter.user.mention}, the **format** of the birthday you provided is **incorrect**! Please provide a birthday in the following format: **YYYY-MM-DD** (i.e. 2007-07-27)")
            else:
